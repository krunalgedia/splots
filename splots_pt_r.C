...
void AddModel(RooWorkspace* ws)
{
    ...
    plot(&frame_bdt_30,BDT_response,-1,1,100, TString("#scale[1.2]{#font[12]{BDT response for 30<jet p_{T}<40}}"),r_p_30_bdt,totalPdf_30,pdf_b_30,pdf_n_30,&TCanvas_bdt_30,&leg_30);

    RooPlot frame_bdt_40;
    TCanvas TCanvas_bdt_40;
    TLegend leg_40;

    plot(&frame_bdt_40,BDT_response,-1,1,100, TString("#scale[1.2]{#font[12]{BDT response for 40<jet p_{T}<60}}"),r_p_40_bdt,totalPdf_40,pdf_b_40,pdf_n_40,&TCanvas_bdt_40,&leg_40);

    RooPlot frame_bdt_60;
    TCanvas TCanvas_bdt_60;
    TLegend leg_60;

    plot(&frame_bdt_60,BDT_response,-1,1,100, TString("#scale[1.2]{#font[12]{BDT response for 60<jet p_{T}<80}}"),r_p_60_bdt,totalPdf_60,pdf_b_60,pdf_n_60,&TCanvas_bdt_60,&leg_60);

    RooPlot frame_bdt_80;
    TCanvas TCanvas_bdt_80;
    TLegend leg_80;

    plot(&frame_bdt_80,BDT_response,-1,1,100, TString("#scale[1.2]{#font[12]{BDT response for 80<jet p_{T}<100}}"),r_p_80_bdt,totalPdf_80,pdf_b_80,pdf_n_80,&TCanvas_bdt_80,&leg_80);

    RooPlot frame_bdt_100;
    TCanvas TCanvas_bdt_100;
    TLegend leg_100;

    plot(&frame_bdt_100,BDT_response,-1,1,100, TString("#scale[1.2]{#font[12]{BDT response for 100<jet p_{T}<120}}"),r_p_100_bdt,totalPdf_100,pdf_b_100,pdf_n_100,&TCanvas_bdt_100,&leg_100);

    RooPlot frame_bdt_120;
    TCanvas TCanvas_bdt_120;
    TLegend leg_120;

    plot(&frame_bdt_120,BDT_response,-1,1,100, TString("#scale[1.2]{#font[12]{BDT response for jet p_{T}>120}}"),r_p_120_bdt,totalPdf_120,pdf_b_120,pdf_n_120,&TCanvas_bdt_120,&leg_120);
